...
class CloudflareChallangeTestSpider(scrapy.Spider):
    ...
    async def start(self):
        ...
        meta = {
            ...
            "playwright_context": f"cf-{proxy.username}" if proxy else "no-proxy",
            ...
            "playwright_page_methods": [
                PageMethod(wait_for_load_cloudflare_default),
            ],
    ...
    def retry_with_curl_cffi(self, response, cookies):
        ...
        return self._curl_cffi_request(
            response.url, curl_pass=1, proxy=proxy, cookies=cookie_dict
        )
    ...
    def _curl_cffi_request(self, url, curl_pass, proxy, cookies=None):
        ...
        return scrapy.Request(
            ...
            meta={
                ...
                **({"proxy": proxy.url, "cf_proxy": proxy} if proxy else {}),
    ...
    def verify_access(self, response):
        ...
        if curl_pass == 1:
            yield self._curl_cffi_request(
                response.url,
                curl_pass=2,
                proxy=response.meta["cf_proxy"]
                if "cf_proxy" in response.meta
                else None,
            )
