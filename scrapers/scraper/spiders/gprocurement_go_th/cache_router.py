...
CACHE_PATTERNS = [
    ...
    re.compile(
        r"^https://[a-z0-9.-]+\.gprocurement\.go\.th/egp-agpc01-web/\d+\.[0-9a-f]{8,20}\.js$"
    ),
]
...
ABORT = ["wps/wcm/connect/bab0932e-03a8-4acd-b03f-39b3d4390a5b/depa.png"]
...
async def cache_route(route):
    ...
    resp = await route.fetch(timeout=300_000)
    ...
    if resp.status != 200:
        ...
        return await route.fulfill(status=resp.status, headers=resp.headers, body=body)
